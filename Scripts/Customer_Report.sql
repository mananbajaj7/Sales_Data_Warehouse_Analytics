...
WITH base_query AS (
  -- 1. Join orders and customers, compute age
  SELECT
    f.order_number,
    f.product_key,
    f.order_date,
    ...
    CAST((strftime('%Y', 'now') - strftime('%Y', c.birthdate)) AS INTEGER) AS age
  FROM fact_sales f
...
-- 3. Final SELECT with segmentation and KPIs
SELECT
  customer_key,
  customer_number,
  customer_name,
  ...
  last_order_date,
  -- recency in months: current date minus last order
  (CAST(strftime('%Y', 'now') AS INTEGER) - CAST(strftime('%Y', last_order_date) AS INTEGER)) * 12 +
  (CAST(strftime('%m', 'now') AS INTEGER) - CAST(strftime('%m', last_order_date) AS INTEGER)) AS recency,
  total_orders,
  total_sales,
  total_quantity,
  total_products,
